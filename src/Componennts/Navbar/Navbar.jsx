...
const Navbar = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const res = await axios.get('https://zozacbackend.onrender.com/api/normaluser/profile', {
          headers: {
  ...
  useEffect(() => {
    ...
    return () => {
      links.forEach(link => link.removeEventListener('click', closeMenuOnClick));
    };
  }, []);
...
{/*           <i className="fas fa-brain"></i> */}
        ...
        <ul className={`nav-menu ${menuOpen ? 'active' : ''}`}>
          ...
          <li><a href="tel: 674274276" target="_blank" rel="noopener noreferrer">Contact</a></li>

          {token && userdata ? (
            <li>
              <Link  className="profildiv">
                <img
                  className="profile"
